---
title: "Dashboard"
description: "Dashboard module for at-a-glance platform health, model usage, and reliability signals in Bud Admin"
---

## 1. Description

The Dashboard module in Bud Admin gives teams a single-pane overview of platform health, model performance, and usage across cloud and local assets. It highlights request volumes, throughput, latency, token usage, and accuracy for the top models (cloud and CPU-first local models from Hugging Face, URL, and disk sources), so you can validate Bud’s GPU-optional deployments at a glance.

The page mirrors Bud’s hybrid model stance: it counts cloud endpoints alongside local checkpoints, breaks down cloud vs local model totals, and surfaces endpoint, cluster, and project readiness in one view. Each chart carries its own time-range toggle (Last 24 Hrs, Last 7 Days, Last 30 Days) so platform, MLOps, and FinOps teams can compare day-over-day, week-over-week, or month-over-month changes without leaving the overview.
...
Cloud endpoints and local models (Hugging Face downloads, URL-based imports, and disk-mounted checkpoints) are counted together, with chips indicating cloud vs local where applicable.
...
### 5.4 Where do latency and throughput numbers come from?

Values are fetched from the `/metrics/analytics` endpoint and normalized via the observability adapter to stay consistent with the Observability module’s metrics.
