...
#### 4.7.1 General tab (deployment detail)
...
#### 4.7.2 Workers tab (deployment detail)

Open **Workers** to see per-worker status, node placement, and utilization.

Sort by worker name, status, node, or utilization to spot unhealthy pods.

Use the filters to focus on specific pools or workers before rolling updates or maintenance.
