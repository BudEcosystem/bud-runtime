...
4. Inspect CPU/GPU/HPU, memory, storage, and network bandwidth gauges to spot capacity or performance issues.
...
2. Monitor summary KPIs for node counts, pods, CPU, memory, disk usage, and health.
3. Review sortable node and pod tables to identify hot spots or repeated restarts.
4. Use the table to compare CPU, memory, and disk utilization across nodes over time.
