...
The Playground module in Bud Admin embeds an interactive, authentication-aware testing surface so teams can try cloud and local models (Hugging Face, URL-based, and disk-based) before routing traffic. The tab appears in the primary navigation alongside Projects, Models, and Clusters, giving one-click access to a secure iframe session seeded with the user’s refresh token. When enabled, it also supports a single-chat mode wired to curated Bud assistants for fast prompt validation without leaving project workflows.
