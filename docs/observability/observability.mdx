---
title: "Observability"
description: "Bud Admin observability module for multi-cluster AI workloads."
---

## 1. Description

The Bud Admin observability module provides end-to-end visibility across CPU, GPU, HPU, and TPU estates that run GenAI services. It unifies runtime health, request analytics, and alerting so platform, MLOps, and FinOps teams can keep hybrid and multi-cloud deployments reliable.
...
Single-pane monitoring for heterogeneous fleets (CPU, GPU, HPU, TPU) with AI-aware metrics such as prompt/response latency, token throughput, and model-specific error rates.

### 2. Deep linkage between infra, models, and routes

Correlate infrastructure signals with model versions, routes, and agents running on Bud so you can trace a spike in latency back to the exact deployment or prompt change.

### 3. FinOps-ready observability

Built-in cost and token accounting per team, project, and client API key, enabling chargeback/showback without exporting data to another system.

### 4. Governed operations for enterprise teams

Role-aware views, audit trails for all observability actions, and exportable reports that align with enterprise compliance and incident review processes.
