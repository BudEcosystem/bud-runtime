---
title: "Evaluations"
description: "Designing, running, and comparing automated model evaluations in Bud Admin"
---

## 1. Description

The Evaluation module in Bud Admin brings together the Experiments tab and the Evaluations Hub so teams can discover benchmarks, inspect results, and launch new runs from a single workspace. The catalog supports name search, trait filters, and modality badges, while detail pages surface description, tags, and tabbed views for Details, Leaderboard, and Evaluations Explorer. Project-level evaluation pages include table summaries plus actions to start another evaluation or export results.

## 2. USPs (Unique Selling Propositions)

### 1. Tabbed evaluation workspace

Experiments and the Evaluations Hub sit under a single tab bar, with a consistent **New experiment** action that opens the run drawer so teams can add studies without leaving the page.

### 2. Discovery through search and traits

Keyword search, live counts, and horizontal trait chips help reviewers narrow the evaluation catalog. Cards render modality badges, tag chips, and meta links (GitHub, paper, website) to keep context close at hand.

### 3. Detail pages with navigable tabs

Evaluation detail pages include breadcrumbs/back controls, a tag expander, and three tabs—Details, Leaderboard, and Evaluations Explorer—so reviewers can pivot between dataset information, ranking tables, and historical runs.

### 4. Experiment lists with flexible filters
...
### 5. Project-level actions and exports

Project evaluation tables surface traits, dataset references, and scores while offering buttons to run another evaluation against a deployment or export CSV results for offline review.
...
### 7. Coverage across cloud and local deployments

Run drawers support targeting cloud-hosted models and local deployments (Hugging Face, URL, or disk-based) so evaluations stay aligned with where models are served.
