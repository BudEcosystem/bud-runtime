...
Experiments, runs, and workflows respect Bud RBAC. Project evaluation tables surface traits, dataset references, and scores while offering buttons to run another evaluation against a deployment or export CSV results for offline review. CSV export buttons on project evaluation lists support audits and downstream reporting.
