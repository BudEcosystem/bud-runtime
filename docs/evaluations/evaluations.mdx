...
1. Select the **Evaluations Explorer** tab to open the run-level table.

2. Confirm the dataset and metric context shown above the table before reviewing rows.

3. Use the **Metric** chip set to verify which metric is applied to the examples in view, sorting or filtering (when available)

   to focus comparisons.

4. Read the **Prompt** column to inspect the dataset requests sourced from the dataset website for traceability, then check the

   **Response** column to see the captured model outputs for each item.

5. Remember that prompts and responses are drawn from the dataset’s published examples to make validation easier, and use table

   pagination or sorting to navigate historical runs without losing context.

### 4.5 Run an evaluation for a project
