...
- Runs history captures model names, trait names, status, start/end timestamps, duration, and per-dataset scores.
...
### 4.3 Review evaluation details (Details tab)
...
1. View the Introduction summary describing the dataset or evaluation scope.
2. Review the bullet lists under Why Run this Eval? and What to Expect? to understand goals and coverage.
...
1. Review Evaluation Values to see bar chart scores sourced from models already evaluated on the platform.
...
### 4.4 Compare models with leaderboards
...
### 4.5 Evaluation Explorer

1. Open an evaluation detail page and select the Evaluation Explorer tab.
4. View each dataset item with its evaluated metric and includes exemplar request and response context sourced from the dataset’s website.
5. Teams can confirm model behavior at a glance rather than inferring results from aggregate scores alone.

### 4.6 Experiments

1. Navigate to Experiments tab.
2. Use search and optional status or model filters to locate an experiment.
3. Review experiment rows for models, traits, status, tags, and created dates.

#### 4.6.1 Add experiment

1. Click + New experiment button.
1. Submit experiment name, description and tags.
2. Experiment is added to the catalog.

#### 4.6.2 View experiment

1. Click on a experiment row or View Experiment button.
2. Experiment detail page is opened.
3. View tokens processed, runs count, runtime and processsing rate.
4. Review the results table for run-specific scores and statuses.
5. Runs history captures model names, trait names, status, start/end timestamps, duration, and per-dataset scores.
6. Benchmark details section summarize average score, duartion and status etc for datasets included in a evaluation run.
7. Current metrics section captures trait-level performance for each dataset.

### 4.7 Run Evaluation

1. Open experiment detail page.
1. Click Run Evaluation button.
1. Submit evaluation name and description.
1. Pick a model from the Model Zoo.
1. Select traits from the list.
1. Choose datasets from the list based on the selected traits.
1. Review selected evaluation details and proceed with run evaluation.
1. Mutiple runs with different configuration can be initaited in an experiment.

### 4.8 Export evaluation results
...
Use the New Experiment button in the Evaluations tab bar to open the run workflow and add a study without leaving the page.
...
Start with Details for scope, Leaderboard for rankings, and Evaluations Explorer for example history and dataset context before making routing decisions.
...
**Q5. Can I target both cloud and local models in evaluations?**

Yes. The run workflow supports cloud deployments alongside local Hugging Face, URL-based, and disk-based models so benchmarking matches where models are hosted.

**Q6. How do I check what traits or modalities an evaluation covers?**
