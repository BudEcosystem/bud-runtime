...
Choose CPU-first or accelerator-backed clusters per deployment and compare performance signals before promoting changes.
...
General, Workers, Settings, Benchmarks, and Model Evaluations tabs provide step-by-step insight into runtime health, policies, and quality checks.

### 5. Fast discovery with search, filters, and action buttons

Filter by status, cluster, or model; search by deployment name; and use action buttons such as Deploy Model, Publish, Run Another Benchmark, and Export to keep workflows efficient.
...
- Sort and filter by status, cluster, or model as needed.
...
- Search and filter by status, model name, cluster name, TPOT, or TTFT.

- Run Another Benchmark to launch new performance runs.

#### 3.3.5 Model Evaluations

- Review evaluation runs with datasets, scores, and timestamps.

- Filter and sort evaluations to validate quality before promotion.
...
### 4.3 Search and filter deployments

1. Use the search box to filter by deployment name.

2. Apply status, model, or cluster filters to narrow the list.

3. Sort columns to prioritize newest or most active deployments.

### 4.4 Open a deployment detail page
...
#### 4.4.2 Workers (local deployments only)
...
2. Search or filter by status, model, cluster, TPOT, or TTFT.

3. Click Run Another Benchmark to start a new run.

4. Compare results to guide routing or scaling decisions.

#### 4.4.5 Model Evaluations

1. Open the Model Evaluations tab.

2. Filter by dataset or score to validate quality.

3. Click Run Another Evaluation to launch a new run.

4. Click Export to download evaluation results.

#### 4.4.6 Publish a deployment
...
### 4.6 Modify permissions for deployments

1. Open the user management page and select a user.

2. Assign view access for users who should browse deployments only.
...
**Q4. Can I benchmark a deployment before routing traffic?**

Yes. Use the Benchmarks tab to run new performance benchmarks and filter results by status, TPOT, or TTFT.

**Q5. How do I publish an endpoint for organizational use?**

Click Publish on the deployment row, set token pricing, and manage pricing later in Publish Details.

**Q6. What if I need to use the endpoint in another app?**

Use the Use this model action to copy ready-made cURL, Python, or JavaScript snippets.

**Q7. Which settings help with reliability?**

Rate limits, retries, and fallback chains in the Settings tab help protect endpoints from traffic spikes and failures.

**Q8. Who can deploy or delete endpoints?**
