...
The Agent module in Bud AI Foundry centralizes how teams build, version, and reuse agents and prompts across projects. It combines a catalog view with project-scoped tables so that platform and product teams can search, filter, and launch multi-step agent workflows without leaving the Bud console. Agents are backed by the same GPU-optional runtime as models and deployments, so you can experiment on CPUs, burst to GPUs/HPUs when needed, and keep governance consistent across environments.
...
- Table navigation routes to agent detail pages for deeper auditability, including forthcoming tabs for cache, logs/traces, and settings as they roll out.
...
### 4.2 Create an agent from the global catalog

1. On the Agents page, click **Agent** (plus button) in the header.
2. The creation drawer starts a workflow for you; pick the project (if prompted), agent type, and backing model or deployment.
...
4. Click **Apply** to refresh the grid; **Reset** clears all filters.
