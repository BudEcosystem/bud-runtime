...
<Check>
  Start with conservative max replica values, then scale after observing load.
</Check>

 <Check>
  Pair autoscaling thresholds with realistic SLO targets.
</Check>

 <Check>
  Use fallback only when secondary deployments are validated.
</Check>
