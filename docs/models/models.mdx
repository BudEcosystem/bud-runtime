...
1. Open the model detail page and select Security Scan.
...
1. Open the model detail page and select Performance tab.
...
1. Open the model detail page and select Quantization.
2. Provide the quantization recipe (bits, group size) and target hardware profile.
...
1. Open the user management page and naviagte to view user page.
2. Assign view access for users who should only browse model listings.
3. Grant manage permissions to users who can add models and perform edits, deletions, or routing .
...
Artifacts require checksum or signature verification, optional SBOM capture, and policy-based approvals before activation. Safety filters and audit logs remain enforced even when running entirely on-prem.

**Q3. Can we prioritize CPU to avoid GPU queues?**

Yes. You can tag backends with CPU-first preferences and set routing rules that favor CPU until SLOs require GPU burst capacity. Autoscaling respects those policies.

**Q4. How do we roll out a new model version safely?**

Use canary or shadow routing with weighted splits, attach evaluation baselines, and monitor analytics. Promote once latency, quality, and safety signals meet thresholds.

**Q5. Does the module support multi-team isolation?**

Yes. Models carry project ownership, and routes can be scoped to teams. API keys, quotas, and analytics remain segmented per project and client.
