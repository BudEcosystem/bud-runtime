...
• Content filters and jailbreak protection, and PII/PHI detection pipelines.
...
1. Open the local model detail page and select Security.
...
2. Choose dataset, prompts, and success thresholds (latency, safety, or quality).
...
1. Open the local model detail page and select Benchmarks.
2. Choose Run Performance Benchmark
3. Type Benchmark name, tags,  hardware targets (CPU/GPU), payload sizes, and concurrency to mirror production.
...
A. Yes. You can tag backends with CPU-first preferences and set routing rules that favor CPU until SLOs require GPU burst capacity. Autoscaling respects those policies.
