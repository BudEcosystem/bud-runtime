...
Bud Model Hub allows you deploy, manage, observe Any huggingface, Private, Public cloud based models (OpenAI, Anthropic, 200+) with any modality (Text, Image, Audio, Embeddings, Actions, Documents etc) on any hardware like GPUs, CPUs, HPUs, NPUs etc from over 12 different hardware vendors. Bud manages the complexity of the cluster selection, config selection, scaling, parallelism settings etc while optimising the deployment to meet your SLO and TCO requirements.

The Model module centralizes how teams register, evaluate, and govern models across cloud and on-prem environments. It brings GPU-optional GenAI to enterprises by mixing elastic cloud endpoints with highly optimized local runtimes, so teams can ship production-grade AI without waiting for scarce accelerator capacity. The module spans LLM, multi-model LLM, embeddings, speech to text, text to speech, action transformers, and document workloads.
...
- Refer to the [[Observability guide](https://budecosystem-b7b14df4.mintlify.app/observability/observability)] for deeper analytics and diagnostics coverage across models.
...
- View result of first-class evaluation runs associated with each model version.
- Compare evaluation results across various deployments, traits and datasets.
- Identify the optimal configuration for the model.

### 3.8 Performance benchmarks

- Performance Benchmark History lists each run with model, cluster, node type, vendor, status, concurrent requests, TPOT, and TTFT, plus search and filter controls.
- Start a new benchmark from the history view using Run Another Benchmark, then track processing status alongside existing entries.
- Use the collected TPOT/TTFT and concurrency metrics to compare how backends perform on different clusters and hardware before selecting deployment targets..
...
3. View the model listing page with counts of deployments, model type, and recommended cluster etc.
...
### 4.9 Run a performance benchmark

1. Open Benchmark History from the Models listing page.
1. Click Run Another Benchmark.
3. Type Benchmark name, tags, description, concurrent request, eval with dataset/configuration.
4. Choose datasets, local model and cluster for the dataset based benchmark run.
5. Provide Max Input Tokens, Max Output Tokens, and select the local model and cluster for the configuration based benchmark run.

### 4.10 Add a quantized model variant
...
### 4.12 Modify permissions for models
