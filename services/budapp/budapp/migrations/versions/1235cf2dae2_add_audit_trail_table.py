...
down_revision: Union[str, None] = "4519dc49ca67"  # "c220a336acbf"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "record_hash",
            sa.String(length=64),
            nullable=True,  # Initially nullable to allow for existing records
    ...
    op.create_index("ix_audit_trail_user_timestamp", "audit_trail", ["timestamp", "user_id"], unique=False)

    # Index for resource history queries
    op.create_index("ix_audit_trail_resource", "audit_trail", ["resource_type", "resource_id"], unique=False)
    ...
    op.create_index("ix_audit_trail_actioned_by", "audit_trail", ["actioned_by"], unique=False)

    # Index for resource_type queries
    op.create_index("ix_audit_trail_resource_type", "audit_trail", ["resource_type"], unique=False)

    # Index for resource_id queries
    op.create_index("ix_audit_trail_resource_id", "audit_trail", ["resource_id"], unique=False)

    # Index for record_hash queries
    op.create_index(
        "ix_audit_trail_record_hash",
        "audit_trail",
        ["record_hash"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index("ix_audit_trail_record_hash", table_name="audit_trail")
