...
async def json_schema_to_pydantic_model(
    schema: Dict[str, Any],
    ...
    for def_name, def_schema in definitions.items():
        if def_name not in existing_models:
            ...
            if "enum" in def_schema:
                ...
                nested_model = await json_schema_to_pydantic_model(
    ...
    for field_name, field_schema in properties.items():
        python_type = await determine_field_type(field_schema, existing_models, field_name)
...
async def determine_field_type(
    field_schema: Dict[str, Any], existing_models: Dict[str, Type[BaseModel]], field_name: str = ""
) -> Any:
    ...
    if "anyOf" in field_schema:
        ...
        for option in field_schema["anyOf"]:
            opt_type = await determine_field_type(option, existing_models, field_name)
    ...
    if isinstance(field_schema.get("type"), list):
        ...
        for type_str in field_schema["type"]:
            if type_str == "null":
                ...
                types.append(await determine_field_type(simple_schema, existing_models, field_name))
    ...
    # Handle arrays with constraints
        ...
        item_type = await determine_field_type(items_schema, existing_models, f"{field_name}_item")
    ...
    # Handle objects (could be dict or nested model)
        ...
        if "properties" in field_schema:
            ...
            if inline_model_name not in existing_models:
                inline_model = await json_schema_to_pydantic_model(
                    field_schema,
                    inline_model_name,
        ...
        if "additionalProperties" in field_schema:
            if field_schema["additionalProperties"] is False:
                ...
                value_type = await determine_field_type(
