    ...
    async def execute_prompt(
        ...
        try:
            ...
            if version:
                ...
                if not redis_key:
                    logger.error(f"Default version not found for prompt_id: {prompt_id}")
                    raise OpenAIResponseException(
                        status_code=404,
            ...
            if not config_json:
                logger.error(f"Prompt configuration not found for key: {redis_key}")
                raise OpenAIResponseException(
                    status_code=404,
            ...
            logger.debug(f"Successfully executed prompt: {prompt_id}")
            ...
            logger.error(f"Validation error during response creation: {str(e)}")
            ...
            logger.error(f"Client error during response creation: {e.message}")
