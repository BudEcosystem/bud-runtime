...
const RoutesComponent: React.FC<RoutesProps> = ({ data }) => {
  ...
  const confirmDelete = (record) => {
    if (record?.status === "deleting" || record?.status === "deleted") {
      errorToast(
        "Deployment is in deleting state, please wait for it to complete",
      );
      return;
    }
    ...
    openConfirm({
      ...
      description:
        "Once you delete the deployment, it will not be recovered. If the deployment code is being used anywhere it wont function. Are you sure?",
      cancelAction: () => {},
      cancelText: "Cancel",
  ...
  return (
    <div className="pb-[60px] pt-[.4rem]">
      ...
      <Table<DataType>
            ...
            render: (text, record) => (
              <div className="min-w-[130px]">
                {hasPermission(PermissionEnum.ModelManage) && (
                  <div
                    className="flex flex-row items-center visible-on-hover"
                    style={{
        ...
        locale={{
          emptyText: (
            <NoDataFount classNames="h-[20vh]" textMessage={`No Routes`} />
          ),
        }}
      />
    </div>
