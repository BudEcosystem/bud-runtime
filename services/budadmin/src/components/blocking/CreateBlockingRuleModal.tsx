...
import { useBlockingRules, BlockingRuleCreate, BlockingRuleType } from '@/stores/useBlockingRules';
...
export const CreateBlockingRuleModal: React.FC<CreateBlockingRuleModalProps> = ({
  visible,
  onClose,
}) => {
      ...
      case 'rate_based_blocking':
        return (
          <>
            ...
            >
              <Input type="number" min={1} placeholder="100" style={{ width: '100%' }} />
            </Form.Item>
            <Form.Item
            ...
            >
              <Input type="number" min={1} placeholder="60" style={{ width: '100%' }} />
            </Form.Item>
          </>
        );
  ...
  const handleConfigChange = (changedValues: any) => {
    ...
    if (changedValues.rule_config) {
      if (changedValues.rule_config.ip_addresses && typeof changedValues.rule_config.ip_addresses === 'string') {
        const ips = changedValues.rule_config.ip_addresses.split('\n').filter((ip: string) => ip.trim());
        form.setFieldValue(['rule_config', 'ip_addresses'], ips);
      }
      if (changedValues.rule_config.patterns && typeof changedValues.rule_config.patterns === 'string') {
        const patterns = changedValues.rule_config.patterns.split('\n').filter((p: string) => p.trim());
    ...
    >
      ...
      >
        ...
        <Form.Item
          name="reason"
          label="Reason"
          rules={[{ required: true, message: 'Please enter reason for this rule' }]}
        >
