        ...
        data?.map((item, index) => {
          ...
          return {
            title:
              !isLast && hasUrl ? (
                <Link href={hasUrl}>
                  <Text_12_400_787B83 className="hover:cursor-pointer hover:text-[#EEEEEE]">
                    {item}
                  </Text_12_400_787B83>
                </Link>
              ) : (
...
// Deploy Model
function DrawerBreadCrumbNavigation({ items: data }: { items: string[] }) {
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() => {
            ...
            document
              .getElementById("bud-drawer")
              ?.classList.toggle("hide-drawer");
            minimizeProcess({
              ...step,
            });
          }}
