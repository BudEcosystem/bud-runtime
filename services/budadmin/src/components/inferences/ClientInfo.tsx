...
import { Descriptions, Tag, Space, Typography, Button, Empty, Avatar } from 'antd';
import {
  CopyOutlined,
  MobileOutlined,
  TabletOutlined,
...
const ClientInfo: React.FC<ClientInfoProps> = ({ data, onCopy }) => {
  ...
  const getBrowserIcon = (browserName?: string) => {
    ...
    if (browser.includes('chrome')) return <ChromeOutlined style={{ color: '#4285f4' }} />;
    if (browser.includes('firefox')) return <Avatar size="small" src="/icons/firefox.png" />;
    if (browser.includes('safari')) return <AppleOutlined style={{ color: '#007aff' }} />;
    if (browser.includes('edge')) return <Avatar size="small" src="/icons/edge.png" />;
    return <QuestionCircleOutlined style={{ color: '#8c8c8c' }} />;
  };
  ...
  const getOSIcon = (osName?: string) => {
    ...
    if (os.includes('windows')) return <WindowsOutlined style={{ color: '#0078d4' }} />;
    if (os.includes('mac') || os.includes('ios')) return <AppleOutlined style={{ color: '#007aff' }} />;
    if (os.includes('android')) return <AndroidOutlined style={{ color: '#3ddc84' }} />;
    if (os.includes('linux')) return <Avatar size="small" src="/icons/linux.png" />;
    return <QuestionCircleOutlined style={{ color: '#8c8c8c' }} />;
  };
  ...
  const getDeviceTypeTag = () => {
    ...
    if (is_bot) return <Tag color="red" icon={<RobotOutlined />}>Bot/Crawler</Tag>;

    const type = device_type?.toLowerCase();
    if (type === 'mobile') return <Tag color="green" icon={<MobileOutlined />}>Mobile</Tag>;
    if (type === 'tablet') return <Tag color="blue" icon={<TabletOutlined />}>Tablet</Tag>;
    if (type === 'desktop') return <Tag color="purple" icon={<DesktopOutlined />}>Desktop</Tag>;

    return <Tag color="default" icon={<QuestionCircleOutlined />}>{device_type || 'Unknown'}</Tag>;
  };
  ...
  const renderOSInfo = () => {
    ...
    return (
      <Space>
        ...
        {data.os_version && (
          <Text type="secondary">v{data.os_version}</Text>
        )}
      </Space>
    );
  };
