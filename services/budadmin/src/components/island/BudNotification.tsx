    ...
    >
      <div className="flex justify-start items-center  max-w-[65%]">
        ...
        <div className="pt-[.3rem] max-w-[94%] ml-[.75rem]">
          <Text_12_400_A4A4A9
            className={`tracking-[-.01rem] max-w-[80%] transition-all duration-300 truncate`}
          >
            {item?.title}
          </Text_12_400_A4A4A9>
          <div className="flex justify-between items-center ">
            <Text_14_600_EEEEEE
              className={`tracking-[-.01rem] transition-all duration-300 truncate `}
            >
              {item?.message}
            </Text_14_600_EEEEEE>
...
export function NotificationsWidget({
  ...
  return (
    <>
      <div
        className={`item rounded-[16px] py-[1.25rem] border border-[#1F1F1F] rounded-[16px]   width-full box-border ${isClosed ? "height-211 " : "h-full "}  item max-h-[90vh] overflow-y-hidden`}
        style={{
          gridRow: `1 / span ${isClosed ? 1 : gridRowCount > 3 ? 3 : gridRowCount}`,
        }}
      >
        <div className="flex justify-between items-center  px-[1.5rem]">
          <Text_24_600_EEEEEE className="tracking-[-.015rem]">
            Notifications
          </Text_24_600_EEEEEE>
          <div className="flex justify-end items-center ">
            {notifications?.length > 1 && (
              <div
                className="w-[1.25rem] h-[1.25rem] rounded-full flex justify-center items-center mr-[.6rem] bg-[#161616] cursor-pointer"
                onClick={toggleList}
              >
                <ChevronDown
            ...
            {notifications?.length > 0 && (
              <div
                className="px-[.4rem] py-[.32rem] rounded-[43px] bg-[#161616] cursor-pointer"
                onClick={async () => {
        ...
        {notifications?.length === 0 || !notifications ? (
          <div className="flex justify-center items-center h-full">
            <Text_12_400_757575 className="py-[1rem]">
