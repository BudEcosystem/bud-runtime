...
function AgentBoxInner({
  ...
  // Track previous values for detecting significant changes
  const prevSelectedDeploymentRef = React.useRef(session?.selectedDeployment?.id);
  const prevSystemPromptRef = React.useRef(session?.systemPrompt);
  const prevInputVarsLengthRef = React.useRef(session?.inputVariables?.length);
  const prevOutputVarsLengthRef = React.useRef(session?.outputVariables?.length);

  // Trigger auto-save when significant session properties change
  React.useEffect(() => {
    if (!session?.id) return;

    const currentDeploymentId = session?.selectedDeployment?.id;
    const currentSystemPrompt = session?.systemPrompt;
    const currentInputVarsLength = session?.inputVariables?.length;
    const currentOutputVarsLength = session?.outputVariables?.length;

    // Check if any significant property changed
    const deploymentChanged = currentDeploymentId !== prevSelectedDeploymentRef.current;
    const systemPromptChanged = currentSystemPrompt !== prevSystemPromptRef.current;
    const inputVarsChanged = currentInputVarsLength !== prevInputVarsLengthRef.current;
    const outputVarsChanged = currentOutputVarsLength !== prevOutputVarsLengthRef.current;

    if (deploymentChanged || systemPromptChanged || inputVarsChanged || outputVarsChanged) {
      // Mark session as changed to trigger auto-save in AgentDrawer
      markSessionChanged(session.id);

      // Update refs
      prevSelectedDeploymentRef.current = currentDeploymentId;
      prevSystemPromptRef.current = currentSystemPrompt;
      prevInputVarsLengthRef.current = currentInputVarsLength;
      prevOutputVarsLengthRef.current = currentOutputVarsLength;
    }
  }, [
    session?.id,
    session?.selectedDeployment?.id,
    ...
    >
      ...
      <div className="topBg text-white p-4 flex justify-between items-center h-[3.625rem] relative sticky top-0 z-10 bg-[#101010] border-b border-[#1F1F1F]">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            className={`w-[1.475rem] h-[1.475rem] p-[.2rem] rounded-[6px] flex justify-center items-center cursor-pointer transition-none ${isToolsOpen ? 'bg-[#965CDE] bg-opacity-20 text-[#B3B3B3] hover:text-[#FFFFFF]' : ''}`}
            onClick={toggleTools}
            style={{ transform: 'none' }}
          >
            <div
            ...
            >
              <Tooltip title={isToolsOpen ? "Close Tools" : "Tools"}>
                <svg
                  ...
                  style={{ transform: 'none', display: 'block' }}
                >
                  <g clipPath={`url(#clip0_959_20489_${index})`}>
                  ...
                  <defs>
                    <clipPath id={`clip0_959_20489_${index}`} fill="currentColor">
