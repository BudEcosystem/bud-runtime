...
const ScoreChart: React.FC<GaugeChartProps> = ({ data }) => {
  ...
  const [scoreChartData, setScoreChartData] =
    useState<GaugeChartProps["data"]>(data);
  ...
  const harmfulness = {
    ...
    value:
      ((scoreChartData.harmfulness.inputValue +
        scoreChartData.harmfulness.outputValue) /
        2) *
      100,
  ...
  const hallucination = {
    ...
    value:
      ((scoreChartData.hallucination.inputValue +
        scoreChartData.hallucination.outputValue) /
        2) *
      100,
  ...
  const sensitiveInformation = {
    ...
    value:
      ((scoreChartData.sensitiveInformation.inputValue +
        scoreChartData.sensitiveInformation.outputValue) /
        2) *
      100,
  ...
  const promptInjection = {
    ...
    value:
      ((scoreChartData.promptInjection.inputValue +
        scoreChartData.promptInjection.outputValue) /
        2) *
      100,
  ...
  return (
    <Grid gap="2" columns="2" rows="repeat(2,108px)" justify="between">
      ...
      <Link
        href={`/projects/${projectId}/endpoint/${endpointId}/hallucination`}
      >
        <GaugeChart props={hallucination} />
      </Link>
      <Link
        href={`/projects/${projectId}/endpoint/${endpointId}/sensitive_info`}
      >
        <GaugeChart props={sensitiveInformation} />
      </Link>
      <Link
        href={`/projects/${projectId}/endpoint/${endpointId}/prompt_injection`}
      >
