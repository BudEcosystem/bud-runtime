...
function ScanResultItem({ data }: { data: ScanResultItemProps }) {
    ...
    >
      <div className="flex justify-between items-center py-[.8rem]">
        <div className="flex justify-start items-center">
          ...
          <Text_12_400_B3B3B3 className="width-82">
            {data.name}
          </Text_12_400_B3B3B3>
          {data.count && (
            <>
...
export default function ScanCompleted() {
  ...
  const scanResult: ScanResult =
    currentWorkflow?.workflow_steps?.security_scan_result ||
    selectedModel?.scan_result;
  const isScanCompleted =
    currentWorkflow?.workflow_steps?.security_scan_result?.model_id;
  ...
  const scanResults: ScanResultItemProps[] = [
    ...
    },
    {
      ...
      name: "Critical",
      count:
        scanResult?.critical_severity_count > 0
          ? scanResult?.critical_severity_count
          : "",
    ...
    },
    {
      ...
      name: "High",
      count:
        scanResult?.high_severity_count > 0
          ? scanResult?.high_severity_count
          : "",
    ...
    },
    {
      ...
      name: "Medium",
      count:
        scanResult?.medium_severity_count > 0
          ? scanResult?.medium_severity_count
          : "",
    ...
    },
    {
      ...
      name: "Low",
      count:
        scanResult?.low_severity_count > 0
          ? scanResult?.low_severity_count
          : "",
      ...
      <BudWraperBox>
        ...
        <BudDrawerLayout>
          <DrawerTitleCard
            ...
            description={
              isScanCompleted
                ? "Looks like the scan is done, below are the results"
                : `Below are the security scan results of ${selectedModel?.name}`
            }
