...
import {
  checkColor,
  getChromeColor,
} from "@/components/ui/bud/dataEntry/TagsInputData";
import {
  Text_12_400_B3B3B3,
  Text_12_400_EEEEEE,
  Text_12_600_EEEEEE,
...
export const MessageContent = forwardRef<HTMLDivElement, MessageContentProps>(
  (props, ref) => {
    ...
    return (
      <div
        className={`MessageContent ${props.messageContentclassNames}`}
        style={{
          zIndex: 100,
        }}
      >
        <div className={`${showAll ? "pl-[0]" : "px-[1.4rem]"} `}>
          ...
          {showAll && (
            <div className="flex justify-end">
              <div
                className="flex justify-end items-center cursor-pointer max-w-[100px] gap-[.35rem] mt-[.2rem]"
                onClick={() => setExpandAll(!expandAll)}
              >
                <Text_12_400_EEEEEE className="leading-[1.2rem]">
        ...
        <div className={`pt-[1.2rem] ${showAll ? "pl-[0]" : "px-[1.4rem]"}`}>
          {displayData.map((item, index) => (
            <div
              key={index}
              className={`flex flex-col justify-end items-end px-[0.9rem] py-[1rem] mb-[.9rem] rounded-[8px] gap-[.5rem] ${showAll ? "bg-[#FFFFFF08]" : "bg-[#161616]"}`}
            >
              <div
                key={index}
                className="flex justify-between items-start gap-[.5rem] w-full"
              >
                ...
                <div className="flex flex-auto justify-between items-center gap-[.8rem] max-w-[90%]">
                  <Text_12_600_EEEEEE className="!leading-[160%] max-w-[90%]">
                    {item.question}
                  </Text_12_600_EEEEEE>
                  {showAll && (
                    <div
                      ...
                      onClick={() =>
                        setOpenDetail((prev) => (prev === index ? null : index))
                      }
                    >
                      <div className="w-[0.9375rem] h-[0.9375rem] ">
                        <Image
              ...
              >
                <div className="flex justify-start items-center gap-[.5rem] w-full pl-[2.7rem]">
                  ...
                  <div className="text-left flex flex-auto  w-full">
                    <Text_12_400_B3B3B3 className="leading-[1.05rem] w-full">
                      {item.description}
                    </Text_12_400_B3B3B3>
                  </div>
                </div>
            ...
            <div ref={ref}>
              {showAll && (
                <>
                  ...
                  <Text_18_600_EEEEEE className="pt-[1.5rem] pb-[1rem]">
                    License
                  </Text_18_600_EEEEEE>
                </>
              )}
        ...
        {!showAll && (
          <div className="flex justify-end pr-[1rem] pb-[.5rem]">
            <div
              className="flex justify-start items-center cursor-pointer duration-[500ms] hover:scale-[1.06]"
              onClick={async (e) => {
                e.stopPropagation();
...
export function MessageTop({
  ...
  return (
    <div
      className={`MessageTop rounded-[6px] pb-[1.25rem] ${classNames}`}
      style={{
        width: "100%",
        // width: '359px',
        background: "#1F1F1F",
    ...
    >
      {(title || description) && (
        <div className="border-b-[1px] border-b-[#757575] px-[1.4rem] py-[1.3rem] pb-[1.2rem] mb-[1.25rem]">
          <Text_14_400_EEEEEE className="leading-[1.24375rem]">
            {title}
          </Text_14_400_EEEEEE>
          <Text_12_400_B3B3B3 className="pt-[1rem] leading-[180%]">
            {description}
          </Text_12_400_B3B3B3>
        </div>
      )}
      ...
      {onClick && (
        <div className={showAll ? "pl-[0]" : "pl-[1.4rem]"}>
          <PrimaryButton
...
export function DropDownContent({
  ...
  return (
    <div
      className={`w-[100%] bg-[#1F1F1F] rounded-[6px] relative ${classNames}`}
      style={{
        zIndex: 1500,
      }}
    >
      {dropMessage && <MessageTop {...dropMessage} />}
      {contentmessage?.show && <MessageContent {...contentmessage} />}
    </div>
  );
}
      ...
      {props.drop ? (
        ...
        >
          {props.showTooltip ? (
            <CustomPopover
              title={copyText}
              contentClassNames="flex justify-center items-center w-full h-full"
              customClassName="flex justify-center items-center w-full h-full"
            >
              <div className="flex justify-center items-center w-full h-full">
                {props.image && <div>{props.image}</div>}
                <div
                  className={`font-[400] leading-[115%] ${props.textClass}`}
                  style={{
                    color: color,
                    fontSize: "0.625rem",
                  }}
