    ...
    async (data) => {
      try {
        ...
        if (
          ...
          data?.message?.payload?.workflow_id !== workflowId
        ) {
          return;
        }
      } catch (error) {
      ...
      if (
        data.message.payload.type === success_payload_type &&
        data.message.payload.category === "internal"
      ) {
        setSteps((steps) => {
          const newSteps = steps.map((step) =>
            data.message.payload.event === step.id &&
        ...
        if (
          data.message.payload.event == "eta" &&
          data.message.payload.content.message
        ) {
      ...
      if (
        data.message.payload.event == "results" &&
        data?.message?.payload?.content?.status === "COMPLETED"
      ) {
        timeout = setTimeout(() => {
          onCompleted();
        }, 3000);
  ...
  return (
    <BudDrawerLayout>
      <div className="flex flex-col	justify-start items-center w-full">
        ...
        {extraInfo && (
          <div className="flex justify-start items-center w-full px-[1.35rem] pt-[1.3rem] pb-[1rem] text-[#B3B3B3] text-[.75rem]">
            {extraInfo}
          </div>
        )}
