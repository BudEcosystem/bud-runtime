    ...
    return (
      <div className="w-full">
        <div className="px-[0.7rem] mb-[1.2rem] pt-[.3rem]">
          <Text_12_400_757575 className="mt-[.55rem] pr-[3rem]">
            Scan your model for security vulnerabilities before you start
            deploying it.
          </Text_12_400_757575>
        </div>
        <div className="flex justify-end items-end px-[0.7rem] pt-[2.6rem] pb-[.3rem]">
          {((hasPermission(PermissionEnum.ModelManage) &&
            currentWorkflow?.workflow_steps?.model_security_scan_events
              ?.status != "PENDING") ||
            !selectedModel.scan_verified) && (
            <PrimaryButton
    ...
    <div className="w-full">
      <div className="px-[0.7rem] mb-[1.2rem]">
        <Text_12_400_757575 className="mt-[.55rem]">
          Model has been scanned and below are the results.
        </Text_12_400_757575>
      </div>
      ...
      {
        <div
          className="flex justify-end items-center mb-[1.2rem] "
          onClick={async () => {
            if (isExpandedViewOpen) return;
...
function Scan({ item }: { item?: any }) {
  ...
  return (
    <div className="w-full">
      <div className="px-[0.7rem] mb-[1.2rem]">
        <Text_12_400_757575 className="mt-[.55rem]">
          {selectedModel?.bud_verified
            ? "Model has been verified by Bud."
            : "Model has not been verified by Bud."}
        </Text_12_400_757575>
      </div>
    </div>
  );
...
export default function ModelVerificationStatus({ item }: { item?: any }) {
  ...
  const tabs = [
    ...
    selectedModel?.provider_type !== "cloud_model"
      ? {
          name: "Security Scan",
          icon: selectedModel?.scan_result
            ? selectedModel.scan_verified
              ? SuccessIcon
              : FailedIcon
            : PendingIcon,
          description:
            "This model has been scanned for security vulnerabilities.",
          content: <SecurityScan />,
        }
      : undefined,
    {
      name: "Evaluations",
      icon: selectedModel?.eval_result
        ? selectedModel.eval_verified
          ? SuccessIcon
          : FailedIcon
        : PendingIcon,
  ...
  return (
    ...
    >
      <div className="absolute-div absolute z-[1250] flex-col justify-start left-[-3%] top-[-0.35rem] mt-[0rem] pt-[2.75rem] rounded-[8px] bg-[#1F1F1F]  w-full min-h-[13.25rem] overflow-hidden shadow-sm shadow-[0_1px_2px_0_#4D4D4D]">
        {tabContent?.name == "Evaluations" && <ComingSoon />}
        {tabContent?.content}
      </div>
