...
function ClusterTags(props: ClusterTagsProps) {
  ...
  const tags: TagListeItem[] = [
    {
      color: "#D1B854",
      name: props.cluster.cpu_count
        ? `${props.cluster.cpu_count || 0} CPU`
        : "",
    },
    {
      color: "#D1B854",
      name: props.cluster.gpu_count
        ? `${props.cluster.gpu_count || 0} GPU`
        : "",
    },
    {
      color: "#D1B854",
      name: props.cluster.hpu_count
        ? `${props.cluster.hpu_count || 0} HPU`
        : "",
    },
    // {
    //     color: '#D1B854',
  ...
  return (
    <div className="flex flex-wrap items-center gap-[.25rem]">
      {!props.hideEndPoints && (
        <Tags
          name={
            props.cluster.total_endpoints_count
              ? props.cluster.total_endpoints_count
              : 0
          }
          color={"#965CDE"}
      ...
      <TagsList
        data={props.maxTags > 0 ? tags.slice(0, props.maxTags) : tags}
      />
