...
export const SelectAdapterModel = () => {
  ...
  const { createAddAdapterWorkflow, setAdapterWorkflow, adapterWorkflow } =
    useDeployModel();
  ...
  useEffect(() => {
    fetchModels({
      ...
      setSelectedModel(
        data.find((item) => item.id == adapterWorkflow?.adapterModelId),
      );
      setModels(data);
    });
  }, [page]);
  ...
  const handleNext = async () => {
    ...
    setAdapterWorkflow({
      ...adapterWorkflow,
      adapterModelId: selectedModel.id,
    });
  ...
  return (
    ...
    >
      <BudWraperBox>
        ...
        <BudDrawerLayout>
          <DrawerTitleCard
            title="Select Adapter Model"
            description="Select the adapter model to add to the endpoint"
          />
          <DeployModelSelect
            models={models}
            ...
            emptyComponent={
              <Text_12_300_EEEEEE>No adapter models found</Text_12_300_EEEEEE>
            }
          >
