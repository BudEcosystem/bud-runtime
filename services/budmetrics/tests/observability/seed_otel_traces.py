"""Seeder for OTel traces test data.

This module loads OTel trace data from JSON fixtures and inserts them
into ClickHouse for testing the otel_traces to InferenceFact pipeline.
"""
...
def format_map_value(value: dict | str) -> str:
    ...
    if isinstance(value, str):
        # Already a string, escape single quotes
...
def seed_otel_traces(
    ...
    Args:
        data_keys: List of data keys to insert (e.g., ['data_1', 'data_2']).
    ...
    for span in all_spans:
        ...
        if isinstance(attrs, dict):
            inf_id = attrs.get("model_inference_details.inference_id")
            if inf_id:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Seed OTel traces from JSON fixture")
    parser.add_argument(
        "--data-keys", "-d",
        nargs="+",
        default=None,
