"""Seeder for prompt distribution OTel traces test data.

This module loads OTel trace data from JSON fixtures and inserts them
into ClickHouse for testing the prompt distribution pipeline.

Can be run standalone from CLI:
    python seed_otel_traces.py --clear --verify
    python seed_otel_traces.py --data-keys concurrent_request_1 concurrent_request_2
"""
...
DATA_FILE = Path(__file__).parent / "data" / "otel_traces_prompt_distribution.json"
...
def seed_otel_traces(
    ...
    Args:
        data_keys: List of data keys to insert (e.g., ['concurrent_request_1', 'concurrent_request_2']).
    ...
    sql_file = Path("/tmp/prompt_distribution_seed.sql")  # nosec B108
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Seed prompt distribution OTel traces from JSON fixture")
    parser.add_argument(
        "--data-keys",
        "-d",
        nargs="+",
        default=None,
        help="Data keys to insert (e.g., concurrent_request_1 concurrent_request_2). Default: all",
    )
