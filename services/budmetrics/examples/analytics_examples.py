...
async def example_1_simple_daily_metrics():
    """Demonstrate simple daily request count for the last 7 days."""
    ...
    async with aiohttp.ClientSession() as session, session.post(ANALYTICS_ENDPOINT, json=payload) as response:
...
async def example_2_multiple_metrics_with_grouping():
    """Demonstrate multiple metrics grouped by model with deltas."""
...
async def example_3_custom_time_intervals():
    """Demonstrate custom 7-day intervals aligned to specific start date."""
...
async def example_4_filtered_metrics():
    """Demonstrate metrics filtered by specific projects and endpoints."""
...
async def example_5_performance_metrics():
    """Demonstrate performance metrics (latency, TTFT, throughput)."""
    ...
    async with aiohttp.ClientSession() as session, session.post(ANALYTICS_ENDPOINT, json=payload) as response:
        ...
        if result.get("items") and result["items"][0]["items"]:
            ...
            for item in latest["items"][:5]:  # Top 5
                ...
                if "latency" in data:
                    lat = data["latency"]
                    print(f"    Latency - Avg: {lat['avg_latency_ms']:.1f}ms, P95: {lat.get('latency_p95', 0):.1f}ms")
...
async def example_7_token_metrics():
    """Demonstrate token usage metrics for cost analysis."""
...
async def example_8_error_analysis():
    """Demonstrate analysis of failure patterns."""
