...
export default function ApiKeysPage() {
    ...
    try {
      ...
      if (response?.data) {
        ...
        const keys = (response.data.credentials || []).map((cred: any) => ({
          ...
          status: !cred.is_active
            ? "revoked"
            : cred.expiry && new Date(cred.expiry) < new Date()
              ? "expired"
              : "active",
          key: cred.key,
        }));
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 200,
      render: (project: any) => (
        <Text_12_400_EEEEEE>{project?.name || "N/A"}</Text_12_400_EEEEEE>
      ),
    ...
    },
    {
      ...
      width: 150,
      render: (text: string) => (
        <Text_12_400_EEEEEE>{text ? formatDate(text) : "-"}</Text_12_400_EEEEEE>
      ),
      sorter: (a: ApiKey, b: ApiKey) => {
      ...
      <div className="boardPageView">
        <div className="boardMainContainer pt-[2.25rem]">
          ...
          <Flex justify="space-between" align="center" className="mb-[2rem]">
            ...
            <PrimaryButton onClick={() => openDrawer("add-new-key")}>
              Create New Key
            </PrimaryButton>
          </Flex>
