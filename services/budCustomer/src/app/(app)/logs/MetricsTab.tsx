...
function ChartCard({ title, subtitle, children, height = '23.664375rem' }: { title: string; subtitle?: string; children: React.ReactNode; height?: string }) {
  return (
    <div className={`bg-[var(--bg-card)] p-[1.55rem] py-[2rem] rounded-[6.403px] border-[1.067px] border-[var(--border-color)] w-full h-[${height}] flex items-center justify-between flex-col`}>
...
function MetricCard({ icon, title, value, subtitle, valueColor = 'var(--text-primary)', showProgress = false, progressValue = 0, progressColor = '#3F8EF7' }: any) {
  return (
    <div className="bg-[var(--bg-card)] p-[1.45rem] pb-[1.2rem] rounded-[6.403px] border-[1.067px] border-[var(--border-color)] min-h-[7.8125rem] flex flex-col items-start justify-between">
...
const MetricsTab: React.FC<MetricsTabProps> = ({ timeRange, inferences, isLoading, viewBy, isActive, filters }) => {
  ...
  return (
    <div className="metrics-container" key={metricsKey}>
      ...
      <Row gutter={[16, 16]} className="mb-6">
        <Col xs={24}>
          <div className="bg-[var(--bg-card)] p-[2.55rem] py-[2rem] rounded-[6.403px] border-[1.067px] border-[var(--border-color)] w-full">
      ...
      <Row gutter={[16, 16]} className="mb-6">
        {viewBy === 'model' && (
          <Col xs={24} md={12}>
            <div className="bg-[var(--bg-card)] p-[1.55rem] py-[2rem] rounded-[6.403px] border-[1.067px] border-[var(--border-color)] h-[22rem]">
              <Text_19_600_EEEEEE className="mb-4 !text-[var(--text-primary)]">Top Models</Text_19_600_EEEEEE>
              <List
                dataSource={metrics.topModels}
                renderItem={(item, index) => (
                  <List.Item className="border-[var(--border-secondary)] py-2">
        ...
        <Col xs={24} md={12}>
          <ChartCard title="Success/Failure Ratio" height="22rem">
            <div className="flex justify-around items-center h-full">
              ...
              <div className="w-px h-32 bg-[var(--border-secondary)]"></div>
              <div className="text-center">
