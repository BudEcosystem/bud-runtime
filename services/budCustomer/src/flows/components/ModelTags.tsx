...
function ModelTags(props: ModelTagsProps) {
    ...
    return <div className="flex flex-wrap items-center gap-[.25rem]">
        ...
        {props.hideTags ? null : <>
            ...
            {!props.limit && props.maxTags && props.maxTags > 0 && tags.slice(0, props.maxTags).map((tag, index) => (
                <Tags
                    key={index}
                    name={tag.name}
            ...
            {!props.limit && !props.maxTags && tags.map((tag, index) => (
                <Tags
                    key={index}
                    name={tag.name}
                    color={tag.color || '#1F1F1F'}
            ...
            {props.maxTags && props.maxTags > 0 && tags.length > props.maxTags && (
                <Popover
                    arrow={false}
                    showArrow={false}
                    content={
