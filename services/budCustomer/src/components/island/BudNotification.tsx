        ...
        <div
            ...
            <div className="flex justify-start items-center max-w-[65%]">
                ...
                <div className="pt-[.3rem] max-w-[94%] ml-[.75rem]">
                    <div className="tracking-[-.01rem] max-w-[80%] transition-all duration-300 truncate text-xs" style={{ color: 'var(--island-text-muted)' }}>
                        {item?.title}
                    </div>
                    <div className="flex justify-between items-center">
                        <div className="tracking-[-.01rem] transition-all duration-300 truncate text-sm font-semibold" style={{ color: 'var(--island-text-primary)' }}>
                            {item?.message}
                        </div>
            ...
            <div className="flex flex-col items-end justify-between h-[2.75rem]">
                <div className="whitespace-nowrap text-xs" style={{ color: 'var(--island-text-disabled)' }}>
                    {item?.time}
                </div>
...
export function NotificationsWidget({
    ...
    return <>
        ...
        <div className={`item rounded-[16px] py-[1.25rem] width-full box-border ${isClosed ? 'height-211 ' : 'h-full '}  item max-h-[90vh] overflow-y-hidden island-card island-theme-aware`}
            style={{
                gridRow: `1 / span ${isClosed ? 1 : gridRowCount > 3 ? 3 : gridRowCount}`
            }}
        >
            <div className="flex justify-between items-center px-[1.5rem]">
                <div className="tracking-[-.015rem] text-2xl font-semibold" style={{ color: 'var(--island-text-heading)' }}>Notifications</div>
                <div className="flex justify-end items-center">
                    {notifications && notifications?.length > 1 && <div className="w-[1.25rem] h-[1.25rem] rounded-full flex justify-center items-center mr-[.6rem] cursor-pointer island-button"
                        onClick={toggleList}
                    >
                        <ChevronDown
                            className="w-[1.1rem] transition-transform duration-300"
                            style={{ color: 'var(--island-text-primary)', transform: isClosed ? 'none' : 'rotate(180deg)' }}
                        />
                    </div>}
                    {notifications && notifications?.length > 0 &&
                        <div className="px-[.4rem] py-[.32rem] rounded-[43px] cursor-pointer island-button" onClick={async () => {
                            await markAllNotificationsAsRead();
                            loadNotifications();
                            setIsClosed(true);
                        }}>
                            <div className="text-xs" style={{ color: 'var(--island-text-primary)' }}>Clear all</div>
                        </div>}
                </div>
            </div>
            {loading && <div className="flex justify-center items-center h-full">
                <div className="text-xs" style={{ color: 'var(--island-text-disabled)' }}>Loading...</div>
            </div>}
            {
                notifications?.length === 0 || !notifications ? <div className="flex justify-center items-center h-full">
                    <div className="py-[1rem] text-xs" style={{ color: 'var(--island-text-disabled)' }}>No Notifications are available</div></div> :
