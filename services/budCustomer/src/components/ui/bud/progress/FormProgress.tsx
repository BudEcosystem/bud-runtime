...
const completed = (
  <li className="flex w-full items-center text-[#965CDE] after:content-[''] after:w-full after:h-0.5 after:border-b after:border-[#965CDE] after:border-2 after:inline-block rounded-[6px] overflow-hidden	"></li>
);
const notCompleted = (
  <li className="flex w-full items-center after:content-[''] after:w-full after:h-0.5 after:border-b after:border-[#1F1F1F] after:border-2 after:inline-block  rounded-[6px] overflow-hidden	"></li>
);
      ...
      {step.progress
        ...
        .map(({ status, title }, index) => {
          if (status === "completed" || status === "inProgress") {
            return (
              <CustomPopover key={title + index} title={title}>
                {completed}
              </CustomPopover>
            );
          } else {
