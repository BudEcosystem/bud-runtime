...
async function testAuthRoutes() {
  ...
  for (const route of routes) {
    try {
      ...
      if (response.status === 200) {
        ...
        if (typeof content === "string") {
          ...
          if (hasAuthElements) {
            ...
            console.log(
              `⚠ ${route} - May not be properly rendering auth content`,
            );
          }
        }
      } else {
      ...
      if (error.code === "ECONNREFUSED") {
        console.log(
          `❌ ${route} - Connection refused. Is the dev server running?`,
        );
      } else {
  ...
  for (const route of oldRoutes) {
    try {
      ...
      if (response.status === 404) {
        ...
        console.log(
          `⚠ ${route} - Status: ${response.status} (May still be accessible)`,
        );
      }
    } catch (error) {
  ...
  console.log(
    "✓ New routes: /auth/login and /auth/register should be accessible",
  );
