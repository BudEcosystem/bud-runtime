...
async function testModelsCatalog() {
  ...
  try {
    ...
    console.log(
      `✓ Total models: ${response.data.total_record || response.data.total_count || 0}`,
    );
    ...
    console.log(
      `✓ Found ${searchResponse.data.models?.length || 0} models matching "GPT"`,
    );
    ...
    console.log(
      "3. Verify you have a valid access token if authentication is required",
    );
    console.log(
      "4. Check that the /models/catalog endpoint exists in the backend",
    );
    process.exit(1);
  }
}
