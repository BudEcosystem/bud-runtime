    ...
    for metric, threshold in thresholds.items():
        ...
        if metric == "success_rate":
            ...
            if current < baseline - threshold:
                regressions.append(
                    f"{metric}: {current:.2f}% (threshold: {baseline - threshold:.2f}%)"
                )
        else:
            ...
            if change_percent > threshold:
                regressions.append(
                    f"{metric}: +{change_percent:.1f}% (threshold: +{threshold}%)"
                )
...
def generate_report(
    ...
    for metric_key, metric_name, unit in metrics_display:
        ...
        if unit == "%":
            report.append(
                f"| {metric_name} | {baseline:.2f}{unit} | {current:.2f}{unit} | {change_str} |"
            )
        elif unit == "req/s":
            report.append(
                f"| {metric_name} | {baseline:.0f} {unit} | {current:.0f} {unit} | {change_str} |"
            )
        else:
            report.append(
                f"| {metric_name} | {baseline:.2f} {unit} | {current:.2f} {unit} | {change_str} |"
