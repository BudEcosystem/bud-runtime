...
module "primary_nixos" {
  ...
  attribute = ".#nixosConfigurations.primary-dev-azure.config.system.build.toplevel"
}
module "primary_disko" {
  source    = "github.com/nix-community/nixos-anywhere//terraform/nix-build"
  attribute = ".#nixosConfigurations.primary-dev-azure.config.system.build.diskoScript"
}
